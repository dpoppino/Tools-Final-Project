...
# In this first portion of code I am retrieving the raw data through the api.
# I made these fucntions below as when I started I was under the impression that we needed data from every month.  Once I found out I only needed
# one month worth of data for each year I kept the function becuase it was already built.
...
final_list = []
# I added the tqdm because I am impatient and needed a way to ensure it was processing.
for i in tqdm(range(15)): #15 is the number of years being pulled.  This would have to be manually updated to add more years
    ...
    # To change the data retrieved to change the items after get in the web address.  There is a list of available data at https://api.census.gov/data/2022/cps/basic/jan/variables.html
    ...
    for i in tqdm(newdata1.index):
        ...
        if a is not 57 and b is not 57 and c is not 0:
            #57 is the code for the parents from the US.  I wasn't looking for that data so I filtered it out.
...
# I saved the variable so I didn't have to continue to call the API everytime.
# Below I copied and pasted this from the variable description from the Census website.
locationdict = {
...
# below I am getting the data ready to be used as the input for the zippopotamus API that we used previously this semester
...
# Saving the data from the zippopotamus work so I didn't have to continue pulling the API all the time.
# Below I copied and pasted this from the variable description from the Census website.
...
#Above I used these data points to save me time from waiting for the API's to process, below I wanted to change the variable name for future use.
api_data = final_df
location_data = USHome

api_data.columns =['Mother', 'Father', 'Code', 'Year', 'Month']


# Below I am merging the data from the data pulled from the Census with data pulled form zippopotamus.
...
# Once I had all the data merged I saved it again but shortly realized I wanted the year as an int.
combined_data['Year'] = combined_data['Year'].astype(int)

# Below I added a row that counted the duplicates and added a quantifiable variable
final_combined_data = combined_data.groupby(combined_data.columns.tolist(), as_index=False).size()
# I wasn't sure if the command above dropped the duplicates so I went ahead and used the command to drop them to minimize the dataframe size.
final_combined_data = final_combined_data.drop_duplicates()
#Below I saved my final data
...
#Below I am starting to narrow down the dataframe for my analysis.
pie_data = final_combined_data[['Father Country', 'Mother Country', 'Location', 'Year', 'size','lat','lon']]
#Below I realized when building my calls for dropdown menus that it didn't like spaces so I changed them to underscores.
...
app.layout = html.Div([
    ...
    html.Div(children='''
        ...
        '''),
    dcc.Graph(id="graph"),
    html.H1(children='2005-2019 Immigration Map'),
    html.Div(children='''
